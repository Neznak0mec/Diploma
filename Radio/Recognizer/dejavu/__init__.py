...
class Dejavu:
    ...
    def fingerprint_directory(self, path: str, extensions: List[str], nprocesses: int = None) -> None:
        ...
        while True:
            try:
                ...
                sid = self.db.insert_song(song_name = song_name,file_hash= file_hash,total_hashes= len(hashes))
    ...
    @staticmethod
    def _fingerprint_worker(arguments, limit, song_name=None):
        ...
        song__name, extension = os.path.splitext(os.path.basename(file_name))

        if song__name == None:
            song_name = song__name
